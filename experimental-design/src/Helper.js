...
const decideOnTheItemsToPresent = allItems => {
  ...
  let targetGroup = allItems
    .sort((a, b) => (a.user_ratings_total > b.user_ratings_total ? -1 : 1))
...
const extractAddress = address_components => {
  ...
  for (const component of addressComponentsToReturn) {
    let data = address_components.find(addressComponent =>
...
const WhatsApp = currentState => {
  let domainToForward = "https://79339156.ngrok.io";
  ...
  urlPrep += `places=${currentState.places
...
const CityMapper = props => {
  ...
  let cityMapperLinkEndname = "&endname=";
  let cityMapperLinkEndAddress = "&endaddress=";
  let result = `${citymapperLinkPre}${props.latitude}${cityMapperLinkMid}${props.longitude}${cityMapperLinkEndname}${props.name}${cityMapperLinkEndAddress}${props.formatted_address}`;
  return result;
      ...
      return new Promise(resolve => {
        service.getDetails(request, (place, status) => {
          if (status === window.google.maps.places.PlacesServiceStatus.OK) {
            ...
            // }
            place.selected = true;
            place.postcode = API.extractPostCode(
            ...
            console.log(
              "Google PlacesService error (may be) code is...: ",
              status
            );
          }
        });
...
const presentationDetailsFromQuery = query => {
  ...
  const postcodesInput = params.get("postcodes")
    ? params.get("postcodes").split(",")
    : null;
  const placesInput = params.get("places")
    ? params.get("places").split(",")
    : null;
  return {
    duration: durationInput,
    postcodes: postcodesInput,
    postcode: postcodeInput,
    places: placesInput
  };
};

const bringNearBySearchResults = (
  locationObj,
  keywordReceived,
  radius = 1000
) => {
  ...
  return executeGoogleNearBySearch(request).then(data => {
    if (data.results.length === 0) {
      console.log("extending radius from: ", radius);
      return bringNearBySearchResults(
        locationObj,
        keywordReceived,
        radius + 2000
      );
...
const bringDistanceMatrix = (
  originsArray,
  destinationsArray,
  booleanForTravelModeForCar
) => {
  return new Promise(resolve => {
...
const decideOnTheMidPointObject = (
  responsesForLongLatForOriginPostcodes,
  algoVersion
) => {
  ...
  if (algoVersion === "v1") {
    ...
    responsesForLongLatForOriginPostcodes.forEach(response => {
      midPointLatitude =
        midPointLatitude +
        response.result.latitude / responsesForLongLatForOriginPostcodes.length;
      midPointLongitude =
        midPointLongitude +
        response.result.longitude /
          responsesForLongLatForOriginPostcodes.length;
    });
...
//   sleep,
  spaceFillerArray,
  contentForEncouragingText,
  decideOnTheMidPointObject,
  bringDistanceMatrix,
  bringNearBySearchResults,
  CityMapper,
  presentationDetailsFromQuery,
  bringPlacesObjects,
//   checkIfNull,
  WhatsApp,
  extractAddress,
  processDuration,
  SearchingPageMessages,
  PostSearchPageMessages,
